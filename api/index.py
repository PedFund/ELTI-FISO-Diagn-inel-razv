...
Версия: Final + Percentages (Графики в %, 1 знак после запятой, имя файла: 5-31_results.xlsx)
"""
...
<head>
    ...
    <title>Диагностика Kidski (Full)</title>
    <style>
...
<body>
    <div class="card">
        ...
        <p class="subtitle">Загрузите Excel или CSV файл диагностики</p>
        <form id="form">
            ...
            <button type="submit" class="btn">Обработать и Скачать</button>
        </form>
        <div id="status"></div>
    </div>
    <script>
        ...
        document.getElementById('form').onsubmit = async (e) => {
            ...
            status.textContent = "⏳ Анализ данных и построение графиков...";
            status.className = "";
            status.style.display = "block";
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    a.download = filename;
                    a.click();
                    status.innerHTML = "✅ <b>Готово!</b><br>Файл <b>" + filename + "</b> скачан.";
                    status.className = "success";
                } else {
                    const text = await res.text();
                    try {
                        const json = JSON.parse(text);
                        status.textContent = "❌ Ошибка: " + (json.error || "Неизвестная ошибка");
                    } catch {
                        status.textContent = "❌ Ошибка сервера: " + text.slice(0, 150);
                    }
                    status.className = "error";
                }
            } catch (err) {
...
def categorize(value):
    ...
    if value < 0.33: return "ниже нормативного"
    elif value <= 0.66: return "нормативный"
    else: return "выше нормативного"
...
def sort_key_town(name):
    ...
    name = str(name).strip()
    if name == "г.Москва": return (0, name)
    if name.startswith("г."): return (1, name)
    if name.startswith(("р.п.", "п.", "пос.")): return (2, name)
    return (6, name)
...
def process_excel(file_content, filename):
    ...
    try:
        ...
        try:
            ...
            raise ValueError(f"Ошибка чтения файла. Проверьте формат. {e}")
    ...
    # Очистка чисел
    numeric_cols = [
        "И5-1Время", "И5-1Ошиб", "И5-2Время", "И5-2Ошиб", "И5-3Время", "И5-3Ошиб",
        "И5-4Время", "И5-4Ошиб", "И5-5Время", "И5-5Ошиб", "И3-1Кольца", "И3-1Ошиб",
        "И3-2Кольца", "И3-2Ошиб", "И3-3Кольца", "И3-3Ошиб", "И3-4Кольца", "И3-4Ошиб",
        "И3-5Кольца", "И3-5Ошиб", "И1-1Сум", "И2Сум", "И4Сум", "И1-2Связн",
        "И1-2РечОформ", "И1-2СамРасс", "В1", "В2", "ЭмоцИдент", "Планир", "Сотруд", "Рефлек"
    ]
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        # Формат для процентов с 1 знаком после запятой
        ...
        if not age_counts.empty:
            ...
            ws = writer.sheets["Возрастные группы"]
            ...
            ch.add_series({
                ...
                "data_labels": {
                    "percentage": True, 
                    "category": True,
                    "num_format": "0.0%"  # Формат: 15.5%
                }
            })
            ws.insert_chart("E2", ch)
        
        # Лист 2: Медианы
        cols_med = ["Когнитивное развитие", "Готовн_УД", "Лог_обобщение", "Перцепция", 
                    "Качество внимания", "Аналит-Синт", "Воображение", "ЭмСоцИнтеллект"]
        med_df = df[cols_med].apply(pd.to_numeric, errors='coerce').median().reset_index()
        med_df.columns = ["Показатель", "Медиана"]
        
        if not med_df.empty:
            med_df.to_excel(writer, sheet_name="Медианы", index=False)
            ws = writer.sheets["Медианы"]
            ws.set_column(0, 0, 25)
            ch_med = wb.add_chart({'type': 'radar', 'subtype': 'filled'})
            ch_med.add_series({
                'name': 'Медианные значения',
                'categories': ['Медианы', 1, 0, len(med_df), 0],
                'values':     ['Медианы', 1, 1, len(med_df), 1],
                'line':       {'color': 'blue'},
                'fill':       {'color': 'blue', 'transparency': 80}
            })
            ch_med.set_title({'name': 'Профиль группы (Медианы)'})
            ch_med.set_style(10)
        ...
        for metr in ["Когнитивное развитие", "Воображение_итог", "ЭмСоцИнтеллект"]:
            ...
            if df.empty:
                t = pd.DataFrame(columns=["Уровень", "Количество", "Доля"])
            else:
                t = df[metr + "_уровень"].value_counts().reindex(
                    ["ниже нормативного", "нормативный", "выше нормативного"], fill_value=0
                ).reset_index()
                t.columns = ["Уровень", "Количество"]
                t["Доля"] = t["Количество"] / len(df)
            ...
            # График столбчатый (ТЕПЕРЬ В ПРОЦЕНТАХ)
            if not t.empty:
                ch = wb.add_chart({'type': 'column'})
                ch.add_series({
                    'name': metr,
                    'categories': [sh_name, 1, 0, 3, 0],
                    # Берем данные из колонки "Доля" (индекс 2), а не "Количество"
                    'values':     [sh_name, 1, 2, 3, 2], 
                    'data_labels': {
                        'value': True,
                        'num_format': '0.0%'  # Подпись в формате 15.5%
                    }
                })
                ch.set_title({'name': f'Распределение: {metr}'})
                # Ось Y тоже в процентах
        ...
        total_orgs = towns["Количество организаций"].sum()
        ...
        # Лист 7: Итого
        df_export = df[["Код", "Возраст", "Когнитивное развитие", "Когнитивное развитие_уровень",
                        "Воображение", "Воображение_итог_уровень", "ЭмСоцИнтеллект", "ЭмСоцИнтеллект_уровень"]]
        df_export.to_excel(writer, sheet_name="Итоговый_Отчет", index=False)
        ws = writer.sheets["Итоговый_Отчет"]
...
def process():
    try:
        ...
        if not f.filename: return jsonify({'error': 'Файл не выбран'}), 400
